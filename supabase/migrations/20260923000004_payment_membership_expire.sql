-- Migration: 20260923000004_payment_membership_expire.sql
-- Description: Update handle_successful_payment RPC to explicitly expire old memberships to cleanly cut off old reminder lifecycles
...
BEGIN
    ...
    -- 3. Calculate Dates for the new membership cycle & identify old membership
    SELECT id, end_date INTO v_active_membership_id, v_active_end_date
    FROM public.memberships
    WHERE member_id = v_member_id AND status IN ('active', 'paused', 'due_soon', 'overdue')
    ...
    -- 5. Expire the old membership to halt its reminders
    IF v_active_membership_id IS NOT NULL THEN
        UPDATE public.memberships
        SET status = 'expired', updated_at = NOW()
        WHERE id = v_active_membership_id;
    END IF;

    -- 6. Update the payment row
