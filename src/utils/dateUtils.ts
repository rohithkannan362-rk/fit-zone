...
export function calculateEndDate(startDate: Date, durationMonths: number): Date {
  const endMonth = addMonths(startDate, durationMonths);
 ...
 */
export function getDaysRelativeToDue(nextDueDate: Date, currentDate: Date = new Date()): number {
 ...
 */
export function formatDate(date: Date | Timestamp, pattern: string = 'dd MMM yyyy'): string {
 ...
 */
export function formatTimestamp(ts: Timestamp | null | undefined, pattern: string = 'dd MMM yyyy'): string {
  if (!ts) return '—';
