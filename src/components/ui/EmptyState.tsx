...
const EmptyState = ({ icon: Icon = Inbox, title, description, action }: EmptyStateProps) => {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="text-sm font-bold uppercase tracking-widest text-white/50 mb-2">{title}</h3>
      {description && (
        <p className="text-[10px] font-medium text-white/30 uppercase tracking-widest max-w-sm">{description}</p>
      )}
      {action && (
        <button
